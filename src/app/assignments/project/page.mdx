...
- Reflect upon your design proposal relative to a principle of human-AI interaction
  ...
  - This must not exceed 150 words.
  - Each project assignment requires reflection relative to a distinct principle
    (i.e., no two project assignments can reflect relative to the same principle).
    You may choose any principle in this first assignment, but this constraint may inform your selection.
  ...
  - Then reflect on your design proposal relative to that principle.
...
Include the following sections:
- Report on 2 formative research sessions with a total of at least 2 participants.
  ...
  - Describe the formative research method used in each session
    (e.g., what you aimed to learn,
    key details of how you applied your method,
    who participated and why you recruited them).
  ...
  - Include specific evidence from your research to support your findings.
    We generally expect this will be quotes or other specific feedback from participants.
- Report on a session of formative research with an AI.
  ...
  - Describe an exploration of trying to use an AI to gain insights into your design problem.
    ...
    As part of this prompting, experiment with describing a persona that the AI should consider.
    Then further experiment with considering a different description or an entirely different persona.
  - This session should use AI to explore your design proposal from a design perspective.
    You may independently choose to consult AI regarding aspects of your implementation,
    but this exploration is focused on design and how AI might supplement your formative research sessions with participants.
...
- Summarize changes to your design based on your formative research findings.
  ...
  - Note you will separately provide updated task scenarios in your design status report.
    That will call attention to where course staff can see these changes or details in your walkthrough.
...
- Reflect upon your formative research relative to a principle of human-AI interaction
  ...
  - Cite the principle that frames your reflection
    ...
    For clarity, cite using the provided identifier and text:     
    <CourseDataLink linkHref={courseData.offering.linkDriveGuidelines} outerComponent="p"/>    
  - Then reflect on your formative research relative to that principle.
...
Submit a Design Status Report as a separate document.
  ...
  - Be sure the report captures your current design for all tasks.
...
implementing at least one of the tasks in your interactive prototype.
...
Include the following sections:
- Describe the tasks currently implemented in your interactive prototype.
  - This does not count against the word limit.
  - Limit this to a single sentence per implemented task.
...
- Summarize the most important changes or new details in your design,
  ...
  - This must not exceed 150 words.
  - Describe how feedback or work toward implementing the tasks prompted you to
  ...
  - If your progress has not required navigating meaningful issues,
    consider that you may need to change or grow the scope of your design.
  - Note you will separately provide updated task scenarios in your design status report.
    That will call attention to where course staff can see these changes or details in your walkthrough.
...
Submit a Design Status Report as a separate document.
  - Follow guidance in <SiteLink linkKey={SiteLinks.assignmentsProjectDesignStatusReports}/>.
  - Be sure the report captures your current design for all tasks.
    Although this assignment only requires implementation of one task, 
    your progress may require or suggest changes in your other task.
  - Be sure implemented tasks include screen captures, updated from prior sketches.

Submit a video demonstrating key functionality of your interactive prototype.
- Must not exceed 3 minutes in length per demonstrated task.
- Include narration explaining how the task is supported in the prototype.
...
- To simplify the matching process, you will both give and receive critique from the same groups.
- Match for peer critique in the provided signup document shared by the entire class.
- You may do this at any point in your process
  (e.g., an early critique might consider the initial prototype’s implementation of one task, 
  a later critique might consider a more polished implementation of both tasks).

Submit two completed peer critiques.
- Author these assignments in the Drive folder corresponding to your project group.
- Create a subfolder called "Revised Prototype", keep all documents in this folder.
- Begin by copying the provided template document.
- Ensure these include your takeaways.

Continue design and development,
implementing both of the tasks in your interactive prototype.
...
- Key Human-AI Interaction Functionality
  - This likely corresponds to your 2 tasks.
...
You may use any desired tool to create your poster.
These templates are designed to print in the correct size
and to convey an appropriate information density, but are otherwise not good examples.
If you use a template, you should heavily modify the template to be unique and represent your project:
...
Continue design and development,
refining details of the two tasks in your interactive prototype.

Conduct at least one usability test with a participant.
The participant may not be a classmate.
If this usability test does not surface at least two key insights,
consider revisiting your method and then conducting another usability test.
...
- Summarize the most important changes or new details in your design,
  ...
  - This must not exceed 250 words.
  - Describe how feedback, work toward implementing the task, or insights from usability testing
...
- As you prepare this video, be mindful of two future needs.
  ...
  - We encourage you to create a video that can be silently played on a loop during the poster showcase
...
- Your submission of this video should be a narrated version intended for course staff,
  but you may want to also gather other video snippets for these potential future needs.
...
- 2 Tasks
  ...
  - Briefly convey a relevant aspect of the implementation.
    ...
    but you can convey what was most important and unique in implementing this task.
...
Presentations should be 5 to 7 minutes in length, which will allow time for discussion and questions.
...
Submit each design status report in a separate document that does not exceed 500 words.
...
- Summarize your current design.
  ...
  - Each design status report will include this summary.
    You may copy it forward between deliverables,
...
- Provide walkthroughs illustrating 2 primary task scenarios.
  ...
  - Use images to provide a step-by-step walkthrough illustratting the scenario.
    - Convey each key step in accomplishing the task with your design.
      These can initially be sketches, mockups, or other illustrations of how a person will experience the scenario.
      As you develop more of your project, these should become images captured from your prototype showing interaction with your design.
    - Convey the walkthrough in enough detail that course staff can understand step-by-step interaction with your design.
      We expect each scenario will include multiple images (e.g., 6 images conveying step-by-step interaction in a scenario).
    - Any text used to provide image captions in the walkthrough does not count against word limits.
  - Highlight key specific changes or details
    that are most important for understanding your current design status.
    - This text must be concise, but does not count against word limits.
    - This might be text alongside a specific image in the walkthrough,
      or it might be higher-level text following the overall walkthrough.
    - Your primary assignment includes a summary of the most important changes or new details in your design.
    - This calls attention to where course staff can see those changes or details in your walkthrough.
  - Describe your current understanding of language model support needed in the proposed interaction.
    - This must not exceed 100 words.
    - Describe when and how your design will or does invoke the language model throughout your scenario.
    - Describe expected input and output for these language model invocations.
    - If understanding your approach to language model support requires additional supporting documentation,
      provide that in the same folder in a human-readable format (e.g., another document, PDFs).
      Appropriately reference this supporting documentation in your description.
      We do not intend this to be a burden,
      so you may decide whether you feel such documentation is helpful for engaging with course staff around your design.
- Ensure walkthroughs remain current throughout your design process.
  - As you develop more of your project, walkthroughs should become images captured from your prototype showing interaction with your design.
  - Changes in a task scenario may require or suggest changes in another task scenario.
    Ensure your design status report conveys your overall design, even while you focus on specific elements.
